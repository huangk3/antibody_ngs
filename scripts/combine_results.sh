...
cat $id/${id}.IGH.Q${qual}.txt |grep "best"| awk 'BEGIN{FS="\t"; OFS="\t"} {print "SampleID", "Type", "aaCDRsCombined", "nCDRsCombined", $0}' > ${temp}
...
for sample in `cat $sampleList`; do 
  for t in IGH IGK IGL; do 
    ...
    if [[ $c -gt 1 ]]; then
	    cat $sample/${sample}.${t}.Q${qual}.txt |grep -v "best" | awk -v s=$sample -v t=$t 'BEGIN{FS="\t"; OFS="\t"} {if ($11 > 9) {print s, t, $5$6$7, $8$9$10,$0}}' >> ${temp}
